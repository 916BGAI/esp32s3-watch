...
void lv_obj_load_anim(lv_obj_t *new_target, lv_obj_t *old_target, lv_scr_load_anim_t anim_type, uint32_t time,
                      uint32_t delay)
{
    lv_disp_t *d = lv_obj_get_disp(new_target);

    lv_anim_t a_new;
    lv_anim_init(&a_new);
    lv_anim_set_var(&a_new, new_target);
    lv_anim_set_time(&a_new, time);
    lv_anim_set_delay(&a_new, delay);

    lv_anim_t a_old;
    lv_anim_init(&a_old);
    lv_anim_set_var(&a_old, old_target);
    ...
    if (old_target)
        lv_event_send(old_target, LV_EVENT_SCREEN_UNLOAD_START, NULL);
    lv_anim_start(&a_new);
    if (old_target)
        lv_anim_start(&a_old);
}