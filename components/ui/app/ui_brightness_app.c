...
static void slider_event_cb(lv_event_t *e)
{
    ...
    lv_snprintf(brightness_app->slider_label_buf,
                sizeof(brightness_app->slider_label_buf), "%d%%",
                (int)lv_slider_get_value(brightness_app->slider));
    lv_label_set_text(brightness_app->slider_label,
                      brightness_app->slider_label_buf);
    lv_obj_align_to(brightness_app->slider_label, brightness_app->slider,
...
void brightness_event_callback(lv_event_t *e)
{
    ...
    options_screen->mutex = true;

    lv_obj_add_flag(options_screen->list, LV_OBJ_FLAG_HIDDEN);
    brightness_app->contanier = lv_obj_create(options_screen->screen);
    lv_obj_set_size(brightness_app->contanier, 240, 250);
    lv_obj_add_style(brightness_app->contanier, &options_screen->style, 0);
    lv_obj_align(brightness_app->contanier, LV_ALIGN_TOP_MID, 0, 30);
    lv_obj_load_anim(brightness_app->contanier, options_screen->list,
                     LV_SCR_LOAD_ANIM_OVER_LEFT, 200, 100);
    lv_label_set_text(options_screen->top_label, "屏 幕 亮 度");
    lv_obj_load_anim(options_screen->top_label, options_screen->top_label,
                     LV_SCR_LOAD_ANIM_FADE_IN, 200, 100);
    ...
    lv_obj_load_anim(options_screen->return_button,
                     options_screen->return_button, LV_SCR_LOAD_ANIM_OVER_RIGHT,
                     200, 100);
    lv_obj_load_anim(options_screen->save_button, options_screen->save_button,
                     LV_SCR_LOAD_ANIM_OVER_LEFT, 200, 100);

    /*Create a transition*/
    static const lv_style_prop_t props[] = { LV_STYLE_BG_COLOR, 0 };
    static lv_style_transition_dsc_t transition_dsc;
    lv_style_transition_dsc_init(&transition_dsc, props, lv_anim_path_linear,
                                 300, 0, NULL);
    ...
    lv_style_set_pad_ver(&brightness_app->style_main,
                         -2); /*Makes the indicator larger*/

    lv_style_init(&brightness_app->style_indicator);
    lv_style_set_bg_opa(&brightness_app->style_indicator, LV_OPA_COVER);
    lv_style_set_bg_color(&brightness_app->style_indicator,
    ...
    lv_slider_set_value(brightness_app->slider, display_brightness_get(),
                        LV_ANIM_ON);
    lv_snprintf(brightness_app->slider_label_buf,
                sizeof(brightness_app->slider_label_buf), "%d%%",
                (int)lv_slider_get_value(brightness_app->slider));
    ...
    lv_obj_add_event_cb(brightness_app->slider, slider_event_cb,
                        LV_EVENT_VALUE_CHANGED, NULL);

    lv_obj_add_style(brightness_app->slider, &brightness_app->style_main,
                     LV_PART_MAIN);
    lv_obj_add_style(brightness_app->slider, &brightness_app->style_indicator,
                     LV_PART_INDICATOR);
    lv_obj_add_style(brightness_app->slider,
                     &brightness_app->style_pressed_color,
                     LV_PART_INDICATOR | LV_STATE_PRESSED);
    lv_obj_add_style(brightness_app->slider, &brightness_app->style_knob,
                     LV_PART_KNOB);
    lv_obj_add_style(brightness_app->slider,
                     &brightness_app->style_pressed_color,
...
void return_save_button_event_callback(lv_event_t *e)
{
    ...
    lv_obj_load_anim(options_screen->top_label, options_screen->top_label,
                     LV_SCR_LOAD_ANIM_FADE_IN, 200, 100);
    lv_obj_load_anim(options_screen->list, brightness_app->contanier,
                     LV_SCR_LOAD_ANIM_OVER_RIGHT, 200, 100);
